# Desktop Rave: Idle Audio Club — press kit

**Desktop Rave** is a Windows desktop idle-management game with an audio-reactive neon club. The **free Steam demo** is scheduled to become public on **30 September 2026 at 18:00 CEST / 16:00 UTC**. **No embargo:** press and creators may publish coverage whenever they choose.

## Start here

- **[Download the essential press pack](https://github.com/victorcash/DesktopRavePressKit/raw/refs/heads/main/Downloads/Desktop-Rave-Demo-Press-Pack.zip)** — copy, facts, reviewer help, four original 4K screenshots, a transparent logo and key art. Exact size and contents: [download details](Downloads/README.md).
- **[Browse the captioned screenshot gallery](Screenshots/README.md)** — previews, original PNGs, suggested captions and credits.
- **[Request a demo preview key or an interview](Press%20Kit/Contact.md)** — one contact, with ready-made email links.
- **[Read the demo FAQ](Press%20Kit/Demo%20FAQ.md)** — progression limits, saves, full-game differences and availability.

Videos are offered separately below; wallpapers are optional [extras](Wallpaper/). You do not need to clone this repository to use the kit.

## Ready-to-publish copy

- [Press release](Press%20Kit/Press%20Release.md) · [plain text](Press%20Kit/Press%20Release.txt) — the free-demo announcement, with a headline, summary, game details, studio boilerplate and media contact.
- [Copy-and-paste press copy](Press%20Kit/Press%20Copy.md) · [plain text](Press%20Kit/Press%20Copy.txt) — a ready-to-use news brief, alternative headlines, a short game description and an About 28 Ducks paragraph.

This is publisher-supplied copy from 28 Ducks. You may copy, excerpt or adapt it for Desktop Rave coverage, including monetized coverage, under the [press-kit license](LICENSE). Choose the format that fits your outlet and edit it to your house style. The `.txt` versions contain plain text for pasting into an editor or CMS; on GitHub, use **Raw** or **Download raw file**.

The announcement copy is dated **26 September 2026** and describes the demo's upcoming release. For later coverage, check availability on Steam and update the tense and dates before publishing.

## Facts, preview access and assets

- [Press facts](Press%20Kit/Press%20Facts.md) — release dates, pitch, demo highlights and music guidance.
- [Demo preview guide](Press%20Kit/Reviewer%20Guide.md) — individual key activation, quick start, controls and recording notes.
- [Recording guide](Press%20Kit/Recording%20Guide.md) — an OBS starting setup, silent capture and troubleshooting.
- [Build and kit updates](Press%20Kit/Updates.md) — dated preview information and kit changes.
- [Screenshots](Screenshots/README.md) and [logos / key art](GraphicAssets/README.md) — coverage assets and suggested credits.
- [Announcement trailer](https://youtu.be/ZPDBeV7jJk4) and [downloadable MP4](https://drive.google.com/file/d/1IGW7Pgp2MChqkPpSixwjEi5Id61g8Oel/view?usp=drive_link).
- [12-second silent gameplay clip](Trailer/desktop-rave-audio-reactive-12s-silent.mp4) — a compact audio-reactive visual excerpt for coverage.
- [Video download guide](Trailer/README.md) — direct download links, formats and music guidance.
- [Steam demo](https://store.steampowered.com/app/4987280/) and [full game](https://store.steampowered.com/app/3952790/).

Press contact: **contact@28ducks.com**. Music heard through external playlists is subject to the track owner's terms; see the guide before publishing captured audio.

## Maintaining this kit

Keep release details consistent with [Press facts](Press%20Kit/Press%20Facts.md) and the Steam pages. Rebuild the plain-text copy, image previews and ZIP together when changing the kit; see [maintenance instructions](tools/README.md).
